...
class EnvironmentDetector {
    ...
    {
        if(!is_file($neonFile)) {
            throw new \Exception(sprintf("Environment configuration file '%s' not found.", $neonFile));
        }
    ...
    private function is($mode)
    {
        if(is_string($this->data[$mode])) {
            return (str_contains($this->domain, $this->data[$mode]) || $this->ip == $this->data[$mode]);
        }

        if(is_array($this->data[$mode])) {
            foreach($this->data[$mode] as $item) {
                if(str_contains($item, $this->domain) || $this->ip == $item) {
                    return TRUE;
                }
            }
            return (in_array($this->domain, $this->data[$mode]));
        }
        else {
            throw new InvalidArgumentException("Invalid data in environment conf file - section " . $mode);
        }
        return false;
    }
